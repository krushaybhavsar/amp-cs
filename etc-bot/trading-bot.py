...
STOCK_DATA = {
    "BOND": {"lowest_sell": 0, "highest_bid": 0, "fair": 1000},
    "VALE": {"lowest_sell": 0, "highest_bid": 0, "fair": 0},
    "VALBZ": {"lowest_sell": 0, "highest_bid": 0, "fair": 0},
    "GS": {"lowest_sell": 0, "highest_bid": 0, "fair": 0},
    "MS": {"lowest_sell": 0, "highest_bid": 0, "fair": 0},
    "WFC": {"lowest_sell": 0, "highest_bid": 0, "fair": 0},
}
...
def on_startup(state_manager):
    ...
    print("Started up")
    state_manager.set_orders_in_symbol_for_direction("BOND", "SELL", {1001: 100 + state_manager.position_for_symbol("BOND")})
    state_manager.set_orders_in_symbol_for_direction("BOND", "BUY", {999: 100 - state_manager.position_for_symbol("BOND")})
...
def on_book(state_manager, book_message):
    ...
    if book_message["type"] == "book":
        ...
        switch = {
            "BOND": handle_bond_message,
            "VALBZ": handle_valbz_message,
            "VALE": handle_vale_message,
            "GS": handle_gs_message,
            "MS": handle_ms_message,
            "WFC": handle_wfc_message,
            "XLF": handle_xlf_message,
        }

        try:
            switch[symbol](state_manager, buy, sell)
        except KeyError:
            pass


def handle_bond_message(state_manager, buy, sell):
    pass


def handle_valbz_message(state_manager, buy, sell):
    if len(buy) == 0 or len(sell) == 0:
        return
    STOCK_DATA["VALBZ"]["lowest_sell"] = sell[0][0]
    STOCK_DATA["VALBZ"]["highest_bid"] = buy[0][0]
    calculate_vale_fair_value()


def calculate_vale_fair_value():
    fairValue = (STOCK_DATA["VALBZ"]["lowest_sell"] + STOCK_DATA["VALBZ"]["highest_bid"]) / 2
    return int(fairValue)


def handle_vale_message(state_manager, buy, sell):
    fairValue = calculate_vale_fair_value()
    print("VALBZ POSTION " + str(state_manager.position_for_symbol("VALBZ")))
    print("FAIR VALUE: ", fairValue)
    for buy_order in buy:
        print(f"Buy: {buy_order[0]}")
        if buy_order[0] > fairValue:
            print("ORDER BETTER THAN FAIR VALUE")
            state_manager.set_orders_in_symbol_for_direction(
                "VALE",
                "SELL",
                {buy_order[0]: 10 + state_manager.position_for_symbol("VALE")},
            )
            state_manager.set_orders_in_symbol_for_direction(
                "VALBZ",
                "BUY",
                {fairValue - 1: 10 - state_manager.position_for_symbol("VALBZ")},
            )
    for sell_order in sell:
        print(f"Sell: {sell_order[0]}")
        if sell_order[0] < fairValue:
            print("ORDER BETTER THAN FAIR VALUE")
            state_manager.set_orders_in_symbol_for_direction(
                "VALE",
                "BUY",
                {sell_order[0]: 10 - state_manager.position_for_symbol("VALE")},
            )
            state_manager.set_orders_in_symbol_for_direction(
                "VALBZ",
                "SELL",
...
def calculate_gs_ms_wfc_fair_value():
    stocks_fv = {"GS": 0, "MS": 0, "WFC": 0}
    for stock in stocks_fv:
        fairValue = (STOCK_DATA[stock]["lowest_sell"] + STOCK_DATA[stock]["highest_bid"]) / 2
        stocks_fv[stock] = int(fairValue)
    return stocks_fv


def handle_gs_message(state_manager, buy, sell):
    if len(buy) == 0 or len(sell) == 0:
        return
    STOCK_DATA["GS"]["lowest_sell"] = sell[0][0]
    STOCK_DATA["GS"]["highest_bid"] = buy[0][0]
    calculate_gs_ms_wfc_fair_value()
    ...
    # #     STOCK_DATA["GS"]["lowest_sell"] = sell[0][0]
    # #     STOCK_DATA["GS"]["highest_bid"] = buy[0][0]
    # #     STOCK_DATA["GS"]["fair"] = (buy[0][0] + sell[0][0]) / 2

    # for buy_order in buy:
    #     if buy_order[0] > gs_fairValue:
    #         print("ORDER BETTER THAN FAIR VALUE")
    #         state_manager.set_orders_in_symbol_for_direction(
    #             "GS",
    #             "SELL",
    #             {buy_order[0]: buy_order[1]},
    #         )
    #     # else:
    #     #     state_manager.set_orders_in_symbol_for_direction(
    #     #         "GS",
    #     #         "BUY",
    #     #         {gs_fairValue - 5: buy_order[1] + 5},
    #     #     )
    # for sell_order in sell:
    #     if sell_order[0] < gs_fairValue:
    #         print("ORDER BETTER THAN FAIR VALUE")
    #         state_manager.set_orders_in_symbol_for_direction(
    #             "GS",
    #             "BUY",
    #             {sell_order[0]: sell_order[1]},
    #         )
    #     # else:
    #     #     state_manager.set_orders_in_symbol_for_direction(
    #     #         "GS",
    #     #         "SELL",
    #     #         {gs_fairValue + 5: sell_order[1] + 5},
    #     #     )


def handle_ms_message(state_manager, buy, sell):
    if len(buy) == 0 or len(sell) == 0:
        return
    STOCK_DATA["MS"]["lowest_sell"] = sell[0][0]
    STOCK_DATA["MS"]["highest_bid"] = buy[0][0]
    calculate_gs_ms_wfc_fair_value()


def handle_wfc_message(state_manager, buy, sell):
    if len(buy) == 0 or len(sell) == 0:
        return
    STOCK_DATA["WFC"]["lowest_sell"] = sell[0][0]
    STOCK_DATA["WFC"]["highest_bid"] = buy[0][0]
    calculate_gs_ms_wfc_fair_value()


def calculate_xlf_fair_value():
    stock_fairs = calculate_gs_ms_wfc_fair_value()
    fairValue = ((2 * stock_fairs["WFC"]) + (3 * stock_fairs["MS"]) + (2 * stock_fairs["GS"]) + 3000) / 10
    return int(fairValue)


def handle_xlf_message(state_manager, buy, sell):
    if len(buy) == 0 or len(sell) == 0:
        return
    xlf_fair = calculate_xlf_fair_value()
    for buy_order in buy:
        if buy_order[0] > xlf_fair:
            print("ORDER BETTER THAN FAIR VALUE")
            state_manager.set_orders_in_symbol_for_direction("XLF", "SELL", {buy_order[0]: buy_order[1]})
    for sell_order in sell:
        if sell_order[0] < xlf_fair:
            print("ORDER BETTER THAN FAIR VALUE")
            state_manager.set_orders_in_symbol_for_direction("XLF", "BUY", {sell_order[0]: sell_order[1]})
...
def on_fill(state_manager, fill_message):
    ...
    print("GOT FILL MESSAGE: ", fill_message)
    if fill_message["symbol"] == "BOND":
        if fill_message["dir"] == "BUY":
            MONEY_MADE -= fill_message["price"] * fill_message["size"]
        else:
            MONEY_MADE += fill_message["price"] * fill_message["size"]

    # if fill_message["symbol"] == "GS"
    #     if fill_message["dir"] == "BUY":
    #         MONEY_MADE -= fill_message["price"] * fill_message["size"]
    #     else:
    #         MONEY_MADE += fill_message["price"] * fill_message["size"]
...
def on_trade(state_manager, trade_message):
    ...
    # print("GOT TRADE MESSAGE: ", trade_message)
